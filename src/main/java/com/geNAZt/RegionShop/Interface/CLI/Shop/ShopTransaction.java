...
public class ShopTransaction extends ShopCommand {
    ...
    public ShopTransaction(RegionShopPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void execute(Player player, String[] args) {
        ...
        PagingList<com.geNAZt.RegionShop.Database.Model.ShopTransaction> transactionPagingList = plugin.getDatabase().find(com.geNAZt.RegionShop.Database.Model.ShopTransaction.class).
                where().
        ...
        List<com.geNAZt.RegionShop.Database.Model.ShopTransaction> transactionList = qryPage.getList();
        ...
        if(transactionList.size() > 0) {
            for(com.geNAZt.RegionShop.Database.Model.ShopTransaction item : transactionList) {
                ItemStack iStack = new ItemStack(item.getItem(), 1);
                String shopName = item.getShop();

                if(item.getType() == com.geNAZt.RegionShop.Database.Model.ShopTransaction.TransactionType.BUY) {
                ...
                if(item.getType() == com.geNAZt.RegionShop.Database.Model.ShopTransaction.TransactionType.SELL) {
                ...
                if(item.getType() == com.geNAZt.RegionShop.Database.Model.ShopTransaction.TransactionType.EQUIP) {
                ...
                if(item.getType() == com.geNAZt.RegionShop.Database.Model.ShopTransaction.TransactionType.ADD) {
                ...
                if(item.getType() == com.geNAZt.RegionShop.Database.Model.ShopTransaction.TransactionType.REMOVE) {
