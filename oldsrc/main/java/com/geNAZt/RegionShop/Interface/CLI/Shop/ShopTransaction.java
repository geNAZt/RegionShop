...
public class ShopTransaction extends ShopCommand {
    ...
    public void execute(Player player, String[] args) {
        ...
        PagingList<Database.Model.ShopTransaction> transactionPagingList = plugin.getDatabase().find(Database.Model.ShopTransaction.class).
                where().
        ...
        List<Database.Model.ShopTransaction> transactionList = qryPage.getList();
        ...
        if(transactionList.size() > 0) {
            for(Database.Model.ShopTransaction item : transactionList) {
                ItemStack iStack = new ItemStack(item.getItem(), 1);
                String shopName = item.getShop();

                if(item.getType() == Database.Model.ShopTransaction.TransactionType.BUY) {
                ...
                if(item.getType() == Database.Model.ShopTransaction.TransactionType.SELL) {
                ...
                if(item.getType() == Database.Model.ShopTransaction.TransactionType.EQUIP) {
                ...
                if(item.getType() == Database.Model.ShopTransaction.TransactionType.ADD) {
                ...
                if(item.getType() == Database.Model.ShopTransaction.TransactionType.REMOVE) {
